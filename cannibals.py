...
class State():
	...
	def isGoalState(self):
		#if all the cannibals and missionaries have been transported across along with the barka
...
def nextStates(current):
	...
	for action in POSSIBLE_ACTIONS:
		...
		if(current.barka==0):
			...
			#deksia population increases
			nextState.deksia[0]+=action[0]
			nextState.deksia[1]+=action[1]
			
			#aristera population decreases
			nextState.aristera[0]-=action[0]
			nextState.aristera[1]-=action[1]
		
		#Moving from deksia to aristera
		elif(current.barka==1):
			
			#deksia population decreases
			nextState.deksia[0]-=action[0]
			nextState.deksia[1]-=action[1]
			
			#aristera population increases
			nextState.aristera[0]+=action[0]
			nextState.aristera[1]+=action[1]
